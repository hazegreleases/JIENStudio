...
class OrganizedLabelingTool(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(import_frame, text="Import Generated", command=self.import_generated).pack(side=tk.LEFT, padx=2)
        ...
        self.generated_tab = self.create_simple_tab("Generated")
        
        self.notebook.add(self.classes_tab, text="Classes")
        self.notebook.add(self.negatives_tab, text="Negatives")
        self.notebook.add(self.generated_tab, text="Generated")
    ...
    def refresh_all_images(self):
        ...
        # Categorize
        all_images = {"Classes": {}, "Negatives": [], "Generated": []}
        ...
        for img_file in os.listdir(images_dir):
            ...
            # Generated detection
            if "generated" in img_file.lower() or "train_sample" in img_file.lower():
                all_images["Generated"].append(img_path)
            elif os.path.exists(label_path) and os.path.getsize(label_path) > 0:
        ...
        for category in ["Negatives", "Generated"]:
    ...
    def handle_delete_key(self, event):
        ...
        elif self.negatives_listbox.curselection() or self.generated_listbox.curselection():
            self.delete_selected_image_from_tab()
    ...
    def import_generated(self):
        ...
        factory_output = os.path.abspath("factory_output")
        
        if not os.path.exists(factory_output):
            messagebox.showwarning("Not Found", "factory_output directory not found.")
            return
        ...
        imported = 0
        for file in os.listdir(factory_output):
            if file.lower().endswith(('.png', '.jpg')):
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                new_name = f"generated_{timestamp}_{file}"
                
                src_img = os.path.join(factory_output, file)
                dst_img = os.path.join(images_dir, new_name)
                shutil.copy2(src_img, dst_img)
                
                # Copy label if exists
                label_file = os.path.splitext(file)[0] + ".txt"
                src_label = os.path.join(factory_output, label_file)
                if os.path.exists(src_label):
                    dst_label = os.path.join(labels_dir, os.path.splitext(new_name)[0] + ".txt")
                    shutil.copy2(src_label, dst_label)
                
                imported += 1
        
        messagebox.showinfo("Imported", f"Imported {imported} generated images.")
        self.refresh_all_images()
